...
// Fully assembled data used by the shared actor spawn path.
...
bool Game::Initialize(SDL_Renderer* renderer) {
    ...
    SpawnActor(ActorKind::Zombie, 220.0f, 320.0f);
    SpawnActor(ActorKind::Skeleton, 820.0f, 320.0f);
    return true;
}
...
void Game::HandleEvent(const SDL_Event& event) {
    ...
    switch (event.button.button) {
        ...
        case SDL_BUTTON_RIGHT:
            SpawnActor(ActorKind::Skeleton, spawnX, spawnY);
            break;
        default:
            break;
    }
...
void Game::Update(const float deltaTime) {
    ...
    UpdateActorDigging(clampedDelta);
...
void Game::UpdateActorDigging(const float deltaTime) {
    auto view = registry_.view<DigBehavior, Velocity, Sprite>();

    // Dig-capable actors run a custom walk -> dig -> walk loop.
    for (const entt::entity entity : view) {
...
void Game::UpdateLocomotion() {
    auto view = registry_.view<Actor, Velocity, Sprite>(entt::exclude<DigBehavior>);
    ...
    for (const entt::entity entity : view) {
        ...
        // Dig-capable actors are excluded because their own system sets Dig/Walk.
...
void Game::RenderActors(SDL_Renderer* renderer) const {
    auto view = registry_.view<Actor, Transform, Renderable, Sprite, Facing>();
