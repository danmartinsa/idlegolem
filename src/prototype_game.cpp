...
constexpr const char* EnemyKindName(const EnemyKind kind) {
    return EnemyTemplateFor(kind).name;
}
...
float RandomFloat(std::mt19937& rng, const float minValue,
...
void FillRect(SDL_Renderer* renderer, const SDL_FRect& rect,
...
void DrawText(SDL_Renderer* renderer, const float x, const float y,
...
void PrototypeGame::BuildStarterBody() {
    ...
    createStarterPart("Stitched Skull", SlotType::Head,
                      Attributes{.reason = 0.4F, .instinct = 1.0F}, 0.0F, 0.0F,
                      0.0F, 0.015F, 0.0F, 0.01F, 0.08F, 0.9F,
                      SDL_Color{105, 105, 124, 255});
    createStarterPart("Patchwork Torso", SlotType::Torso,
                      Attributes{.vitality = 2.0F}, 24.0F, 0.0F, 0.0F, 0.0F,
                      1.4F, 0.0F, 0.07F, 1.2F, SDL_Color{126, 88, 82, 255});
    createStarterPart("Scrap Left Arm", SlotType::LeftArm,
                      Attributes{.might = 1.0F}, 0.0F, 2.0F, -0.03F, 0.0F, 0.0F,
                      0.0F, 0.09F, 0.8F, SDL_Color{142, 112, 98, 255});
    createStarterPart("Scrap Right Arm", SlotType::RightArm,
                      Attributes{.might = 1.0F}, 0.0F, 2.0F, -0.03F, 0.0F, 0.0F,
                      0.0F, 0.09F, 0.8F, SDL_Color{142, 112, 98, 255});
    createStarterPart("Bent Left Leg", SlotType::LeftLeg,
                      Attributes{.agility = 1.0F}, 0.0F, 0.0F, -0.02F, 0.0F,
                      0.0F, 0.015F, 0.09F, 0.8F, SDL_Color{108, 126, 100, 255});
    createStarterPart("Bent Right Leg", SlotType::RightLeg,
                      Attributes{.agility = 1.0F}, 0.0F, 0.0F, -0.02F, 0.0F,
                      0.0F, 0.015F, 0.09F, 0.8F, SDL_Color{108, 126, 100, 255});
    createStarterPart("Donor Heart", SlotType::Heart,
                      Attributes{.vitality = 1.0F}, 18.0F, 0.0F, 0.0F, 0.0F,
                      0.0F, 0.0F, 0.08F, 1.0F, SDL_Color{172, 72, 84, 255});
    createStarterPart("Threadbare Lungs", SlotType::Lungs,
                      Attributes{.vitality = 0.5F, .agility = 0.7F}, 0.0F, 0.0F,
                      -0.01F, 0.0F, 0.0F, 0.01F, 0.08F, 0.8F,
...
void PrototypeGame::SpawnEnemyIfNeeded(const float deltaSeconds) {
    ...
    const auto& runState = registry_.get<RunState>(player_);
...
void PrototypeGame::SpawnEnemy(const int tier, const bool elite) {
    ...
    const float scale =
        1.0F + static_cast<float>(tier - 1) * 0.35F +
        static_cast<float>(registry_.get<RunState>(player_).depth - 1) * 0.06F +
        (elite ? 0.75F : 0.0F);

    currentEnemy_ = registry_.create();
    registry_.emplace<Name>(currentEnemy_,
                            elite ? std::string("Elite ") + enemyTemplate.name
                                  : std::string(enemyTemplate.name));
    registry_.emplace<EnemyTag>(currentEnemy_);
    registry_.emplace<Enemy>(currentEnemy_,
                             Enemy{.kind = kind, .tier = tier, .elite = elite});
    registry_.emplace<CombatStats>(
        currentEnemy_,
        CombatStats{
            .attackPower = enemyTemplate.attack * scale,
            .attackInterval = Clamp(enemyTemplate.attackInterval -
                                        static_cast<float>(tier - 1) * 0.04F,
                                    0.7F, 1.8F),
            .critChance = elite ? 0.12F : 0.06F,
            .critMultiplier = elite ? 1.75F : 1.5F,
            .armor = enemyTemplate.armor + static_cast<float>(tier - 1) * 0.7F +
                     (elite ? 1.5F : 0.0F),
            .evasion = Clamp(enemyTemplate.evasion + (elite ? 0.04F : 0.0F),
                             0.0F, 0.28F),
        });
    registry_.emplace<Health>(currentEnemy_,
                              Health{.current = enemyTemplate.health * scale,
                                     .maximum = enemyTemplate.health * scale});
    registry_.emplace<CombatTimer>(
        currentEnemy_, CombatTimer{RandomFloat(rng_, 0.25F, 0.55F)});
    ...
    logLine << "Encounter " << runState.encounters << ": "
            << registry_.get<Name>(currentEnemy_).value;
    AddLog(logLine.str());
}
...
void PrototypeGame::RecalculatePlayerStats() {
    ...
    for (const entt::entity slotEntity : body.slots) {
        if (slotEntity == entt::null || !registry_.valid(slotEntity)) {
            continue;
        }

        const BodyPart& part = registry_.get<BodyPart>(slotEntity);
    ...
    stability.capacity =
        6.0F + current.reason * 0.85F + current.vitality * 0.45F;
    ...
    CombatStats stats{};
    stats.attackPower =
        std::max(3.5F, 6.0F + current.might * 2.45F + bonusAttack -
                           stability.overload * 1.15F);
    stats.attackInterval = Clamp(1.45F - current.agility * 0.05F +
                                     intervalBonus + stability.overload * 0.05F,
                                 0.45F, 1.8F);
    stats.critChance = Clamp(0.03F + current.instinct * 0.01F + critBonus -
                                 stability.overload * 0.01F,
                             0.01F, 0.45F);
    stats.critMultiplier = 1.45F + current.reason * 0.035F;
    stats.armor = std::max(0.0F, current.vitality * 0.52F + armorBonus -
                                     stability.overload * 0.4F);
    stats.evasion = Clamp(
    ...
    health.maximum =
        std::max(30.0F, 72.0F + current.vitality * 16.0F + bonusHealth -
...
void PrototypeGame::UpdateCombat(const float deltaSeconds) {
    ...
    while (!runFailed_ && currentEnemy_ != entt::null &&
...
void PrototypeGame::ResolveAttack(const entt::entity attacker,
...
void PrototypeGame::HarvestCorpse(const entt::entity corpseEntity) {
    ...
    if (dropPart) {
        ...
        rarity = std::max(
            0, rarity + (RandomFloat(rng_, 0.0F, 1.0F) < 0.25F ? 1 : 0));
        rarity = std::min(rarity, 5);
        const float scale = 1.0F + static_cast<float>(corpse.tier - 1) * 0.35F +
                            static_cast<float>(rarity) * 0.28F;
        ...
        const auto evaluatePartScore = [this](const entt::entity entity) {
            ...
            return freshnessFactor *
                   (part.attackBonus * 4.5F + part.healthBonus * 0.32F +
                    ...
                    part.attributeBonus.reason * 1.4F +
                    part.attributeBonus.instinct * 1.8F +
...
void PrototypeGame::DissolveRun() {
    ...
    std::ostringstream logLine;
    logLine << "Dissolve complete: +" << payout
            << " Primordial Essence banked.";
    AddLog(logLine.str());
}

void PrototypeGame::AddLog(const std::string& message) {
    eventLog_.push_front(message);
    while (eventLog_.size() > 8) {
        eventLog_.pop_back();
    }
}
...
void PrototypeGame::UpdateWindowTitle() {
    ...
    title << "idlegolem | prototype | depth " << runState.depth << " | kills "
          << resources.kills << " | biomass " << resources.biomass
          << " | bone meal " << resources.boneMeal << " | essence "
...
void PrototypeGame::RenderBackdrop() const {
    FillRect(renderer_, SDL_FRect{0.0F, 0.0F, 1280.0F, 720.0F},
             SDL_Color{18, 14, 22, 255});
    FillRect(renderer_, SDL_FRect{0.0F, 0.0F, 1280.0F, 200.0F},
             SDL_Color{28, 16, 24, 255});
}
...
void PrototypeGame::RenderResourcePanel(const SDL_FRect& panel) const {
    ...
    DrawText(
        renderer_, panel.x + 16.0F, panel.y + 10.0F,
    ...
    resourceLine << "Zone: Charnel Pits   Depth: " << runState.depth
                 << "   Kills: " << resources.kills
                 << "   Biomass: " << resources.biomass
                 << "   Bone Meal: " << resources.boneMeal
                 << "   Run Essence: " << resources.essence
                 << "   Banked Essence: " << bankedEssence_;
    DrawText(renderer_, panel.x + 16.0F, panel.y + 30.0F,
             SDL_Color{206, 182, 178, 255}, resourceLine.str());

    std::ostringstream statusLine;
    statusLine << "Grafts: " << resources.grafts
               << "   Salvaged: " << resources.salvaged
               << "   Elite in: " << std::max(0, runState.eliteCountdown)
               << "   Stability " << FormatNumber(stability.demand) << "/"
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 50.0F,
...
void PrototypeGame::RenderPlayerPanel(const SDL_FRect& panel) const {
    ...
    DrawText(renderer_, panel.x + 246.0F, panel.y + 36.0F,
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 60.0F,
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 78.0F,
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 96.0F,
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 114.0F,
             stability.overload > 0.0F ? SDL_Color{214, 108, 98, 255}
...
void PrototypeGame::RenderEncounterPanel(const SDL_FRect& panel) const {
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 12.0F,
             SDL_Color{232, 216, 216, 255}, "Encounter lane");

    FillRect(renderer_,
             SDL_FRect{panel.x + 38.0F, panel.y + 94.0F, 120.0F, 140.0F},
             SDL_Color{104, 58, 64, 255});
    DrawRect(renderer_,
             SDL_FRect{panel.x + 38.0F, panel.y + 94.0F, 120.0F, 140.0F},
             SDL_Color{226, 218, 214, 255});
    DrawText(renderer_, panel.x + 50.0F, panel.y + 154.0F,
             SDL_Color{24, 24, 24, 255}, "YOU");

    if (currentEnemy_ != entt::null && registry_.valid(currentEnemy_)) {
        const Name& name = registry_.get<Name>(currentEnemy_);
        const Health& health = registry_.get<Health>(currentEnemy_);
        const CombatStats& stats = registry_.get<CombatStats>(currentEnemy_);
        const Enemy& enemy = registry_.get<Enemy>(currentEnemy_);
        const EnemyTemplate enemyTemplate = EnemyTemplateFor(enemy.kind);

        FillRect(renderer_,
                 SDL_FRect{panel.x + 560.0F, panel.y + 90.0F, 136.0F, 148.0F},
                 enemyTemplate.color);
        DrawRect(renderer_,
                 SDL_FRect{panel.x + 560.0F, panel.y + 90.0F, 136.0F, 148.0F},
                 SDL_Color{226, 218, 214, 255});
        DrawText(renderer_, panel.x + 576.0F, panel.y + 154.0F,
                 SDL_Color{18, 18, 18, 255}, enemy.elite ? "ELITE" : "ENEMY");

        DrawText(renderer_, panel.x + 190.0F, panel.y + 54.0F,
                 SDL_Color{220, 206, 194, 255}, name.value);
        DrawBar(renderer_,
                SDL_FRect{panel.x + 190.0F, panel.y + 78.0F, 330.0F, 14.0F},
                health.current / std::max(1.0F, health.maximum),
                SDL_Color{182, 86, 96, 255}, SDL_Color{84, 38, 48, 255});

        std::ostringstream lineOne;
        lineOne << "Tier " << enemy.tier << "   HP "
                << static_cast<int>(std::round(health.current)) << "/"
                << static_cast<int>(std::round(health.maximum));
        DrawText(renderer_, panel.x + 190.0F, panel.y + 102.0F,
                 SDL_Color{204, 190, 184, 255}, lineOne.str());

        std::ostringstream lineTwo;
        lineTwo << "ATK " << FormatNumber(stats.attackPower) << "  ARM "
                << FormatNumber(stats.armor) << "  EVA "
                << static_cast<int>(std::round(stats.evasion * 100.0F)) << "%";
        DrawText(renderer_, panel.x + 190.0F, panel.y + 122.0F,
                 SDL_Color{194, 180, 208, 255}, lineTwo.str());

        DrawText(
            renderer_, panel.x + 190.0F, panel.y + 160.0F,
            SDL_Color{154, 172, 154, 255},
            "Auto-combat is active. Better parts are grafted automatically.");
    } else if (currentCorpse_ != entt::null &&
               registry_.valid(currentCorpse_)) {
        const Corpse& corpse = registry_.get<Corpse>(currentCorpse_);
        DrawText(renderer_, panel.x + 190.0F, panel.y + 72.0F,
                 SDL_Color{212, 206, 194, 255},
                 registry_.get<Name>(currentCorpse_).value);
        DrawText(
            renderer_, panel.x + 190.0F, panel.y + 96.0F,
            SDL_Color{164, 194, 148, 255},
            "Harvesting remains for Biomass, Bone Meal, and body parts...");
        DrawBar(renderer_,
                SDL_FRect{panel.x + 190.0F, panel.y + 126.0F, 330.0F, 14.0F},
                1.0F - corpse.harvestTimer / (corpse.elite ? 1.2F : 0.8F),
                SDL_Color{114, 168, 98, 255}, SDL_Color{52, 72, 48, 255});
    } else if (runFailed_) {
        DrawText(renderer_, panel.x + 190.0F, panel.y + 88.0F,
                 SDL_Color{220, 118, 108, 255},
                 "Run failure: the body collapsed under enemy pressure.");
        DrawText(renderer_, panel.x + 190.0F, panel.y + 112.0F,
                 SDL_Color{202, 192, 184, 255},
                 "Press R to restart the run or E to dissolve it into banked "
                 "Essence.");
    } else {
        DrawText(renderer_, panel.x + 190.0F, panel.y + 88.0F,
                 SDL_Color{208, 198, 188, 255},
                 "Scouting the Charnel Pits for the next encounter...");
        DrawBar(renderer_,
                SDL_FRect{panel.x + 190.0F, panel.y + 118.0F, 330.0F, 14.0F},
                1.0F - enemySpawnDelay_ / 0.95F, SDL_Color{104, 152, 194, 255},
                SDL_Color{46, 60, 82, 255});
    }

    DrawText(renderer_, panel.x + 16.0F, panel.y + 278.0F,
             SDL_Color{194, 190, 172, 255}, "Prototype loop");
    DrawText(renderer_, panel.x + 16.0F, panel.y + 298.0F,
             SDL_Color{170, 168, 162, 255}, "1. Auto-fight a single target");
    DrawText(renderer_, panel.x + 16.0F, panel.y + 316.0F,
             SDL_Color{170, 168, 162, 255}, "2. Auto-harvest the corpse");
    DrawText(renderer_, panel.x + 16.0F, panel.y + 334.0F,
             SDL_Color{170, 168, 162, 255},
             "3. Auto-graft upgrades and salvage weaker parts");
    DrawText(renderer_, panel.x + 16.0F, panel.y + 352.0F,
             SDL_Color{170, 168, 162, 255},
             "4. Parts decay, enemies scale, elite waves arrive");
}
