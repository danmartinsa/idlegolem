...
std::string BuildSkillNodeSummary(const SkillNodeDefinition& definition) {
    ...
    if (effect.critChanceBonus > 0.0F) {
        append("Crit +" +
               std::to_string(static_cast<int>(std::round(effect.critChanceBonus * 100.0F))));
    }
    ...
    if (effect.evasionBonus > 0.0F) {
        append("Eva +" +
               std::to_string(static_cast<int>(std::round(effect.evasionBonus * 100.0F))));
    }
    ...
    if (effect.decayMultiplierBonus < 0.0F) {
        append("Decay -" +
               std::to_string(static_cast<int>(std::round(-effect.decayMultiplierBonus * 100.0F))) +
               "%");
    }
    if (effect.harvestMultiplierBonus > 0.0F) {
        append(
            "Yield +" +
            std::to_string(static_cast<int>(std::round(effect.harvestMultiplierBonus * 100.0F))) +
            "%");
    }
    if (effect.harvestSpeedMultiplierBonus > 0.0F) {
        append("Harvest +" +
               std::to_string(
                   static_cast<int>(std::round(effect.harvestSpeedMultiplierBonus * 100.0F))) +
               "%");
    }
    if (effect.levelHealPercentBonus > 0.0F) {
        append("Lvl heal " +
               std::to_string(static_cast<int>(std::round(effect.levelHealPercentBonus * 100.0F))) +
               "%");
    }
...
void FleshgolemGame::RenderTitleScreen() const {
    ...
    const SDL_FRect leftColumn{panel.x + 28.0F, panel.y + 34.0F, 386.0F, 360.0F};
    const SDL_FRect rightColumn{panel.x + 446.0F, panel.y + 34.0F, 406.0F, 278.0F};
    const SDL_FRect footerPanel{panel.x + 446.0F, panel.y + 332.0F, 406.0F, 168.0F};
    const SDL_FRect startButton = TitleStartButtonRect();

    DrawPanelChrome(renderer_, panel, SDL_Color{24, 18, 24, 228}, SDL_Color{206, 188, 174, 255},
                    SDL_Color{146, 74, 86, 255});
    DrawPanelChrome(renderer_, leftColumn, SDL_Color{28, 20, 28, 220},
                    SDL_Color{124, 104, 108, 255}, SDL_Color{178, 118, 96, 255});
    DrawPanelChrome(renderer_, rightColumn, SDL_Color{24, 22, 30, 220},
                    SDL_Color{112, 106, 128, 255}, SDL_Color{110, 136, 174, 255});
    DrawPanelChrome(renderer_, footerPanel, SDL_Color{26, 22, 28, 220},
                    SDL_Color{112, 106, 128, 255}, SDL_Color{96, 134, 116, 255});

    DrawTag(renderer_, SDL_FRect{leftColumn.x + 12.0F, leftColumn.y + 16.0F, 134.0F, 24.0F},
            SDL_Color{184, 120, 98, 255}, SDL_Color{236, 228, 220, 255}, "SDL3 + EnTT Demo");

    DrawText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 54.0F, SDL_Color{242, 226, 214, 255},
             "FLESHGOLEM");
    DrawWrappedText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 82.0F, leftColumn.w - 32.0F, 2,
                    16.0F, SDL_Color{206, 188, 176, 255},
                    "A run-based grafting crawl with auto-combat.");

    DrawText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 124.0F, SDL_Color{220, 208, 198, 255},
             "This demo now includes:");
    DrawWrappedText(renderer_, leftColumn.x + 24.0F, leftColumn.y + 148.0F, leftColumn.w - 48.0F, 2,
                    16.0F, SDL_Color{190, 184, 178, 255},
                    "- title flow, polish, and clearer visual hierarchy");
    DrawWrappedText(renderer_, leftColumn.x + 24.0F, leftColumn.y + 180.0F, leftColumn.w - 48.0F, 2,
                    16.0F, SDL_Color{190, 184, 178, 255},
                    "- three zones, elite waves, bosses, and level-based skill growth");
    DrawWrappedText(renderer_, leftColumn.x + 24.0F, leftColumn.y + 212.0F, leftColumn.w - 48.0F, 2,
                    16.0F, SDL_Color{190, 184, 178, 255},
                    "- meta Essence, Research, generated assets, and audio");
    DrawWrappedText(renderer_, leftColumn.x + 24.0F, leftColumn.y + 244.0F, leftColumn.w - 48.0F, 2,
                    16.0F, SDL_Color{190, 184, 178, 255},
                    "- the original auto-combat and graft loop underneath");

    DrawText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 290.0F, SDL_Color{222, 204, 166, 255},
             "Run summary");
    std::ostringstream metaLine;
    metaLine << "Banked Essence " << bankedEssence_ << "   Runs " << completedRuns_
             << "   Last payout " << lastPayout_;
    DrawWrappedText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 314.0F, leftColumn.w - 32.0F, 2,
                    16.0F, SDL_Color{214, 198, 172, 255}, metaLine.str());
    DrawWrappedText(renderer_, leftColumn.x + 16.0F, leftColumn.y + 346.0F, leftColumn.w - 32.0F, 2,
                    16.0F, SDL_Color{188, 196, 188, 255},
                    "Space pause  K tree  R restart  E dissolve  N scout");

    DrawText(renderer_, rightColumn.x + 16.0F, rightColumn.y + 18.0F, SDL_Color{232, 220, 214, 255},
             "Demo structure");
    DrawTag(renderer_, SDL_FRect{rightColumn.x + 16.0F, rightColumn.y + 50.0F, 112.0F, 24.0F},
            SDL_Color{110, 144, 110, 255}, SDL_Color{230, 228, 222, 255}, "1. Start weak");
    DrawTag(renderer_, SDL_FRect{rightColumn.x + 146.0F, rightColumn.y + 50.0F, 118.0F, 24.0F},
            SDL_Color{122, 104, 162, 255}, SDL_Color{230, 228, 222, 255}, "2. Harvest");
    DrawTag(renderer_, SDL_FRect{rightColumn.x + 282.0F, rightColumn.y + 50.0F, 110.0F, 24.0F},
            SDL_Color{184, 128, 72, 255}, SDL_Color{230, 228, 222, 255}, "3. Upgrade");

    DrawWrappedText(renderer_, rightColumn.x + 16.0F, rightColumn.y + 98.0F, rightColumn.w - 32.0F,
                    3, 20.0F, SDL_Color{196, 188, 182, 255},
                    "March through the Charnel Pits, Ash Chapel, and Black Cathedral, choosing one "
                    "skill path as level-ups grant points to spend in the tree.");
    DrawWrappedText(renderer_, rightColumn.x + 16.0F, rightColumn.y + 178.0F, rightColumn.w - 32.0F,
                    2, 16.0F, SDL_Color{170, 186, 204, 255},
                    "Mouse and keyboard are both supported.");
    DrawWrappedText(renderer_, rightColumn.x + 16.0F, rightColumn.y + 210.0F, rightColumn.w - 32.0F,
                    2, 16.0F, SDL_Color{170, 186, 204, 255},
                    "Spend points with mouse clicks or keyboard navigation in the tree.");
    DrawWrappedText(renderer_, rightColumn.x + 16.0F, rightColumn.y + 242.0F, rightColumn.w - 32.0F,
                    2, 16.0F, SDL_Color{170, 186, 204, 255},
                    "Runs can end in victory, collapse, or dissolve.");

    DrawText(renderer_, footerPanel.x + 16.0F, footerPanel.y + 18.0F, SDL_Color{232, 220, 214, 255},
             "Awaken the run");
    DrawWrappedText(renderer_, footerPanel.x + 16.0F, footerPanel.y + 44.0F, footerPanel.w - 32.0F,
                    2, 16.0F, SDL_Color{184, 190, 182, 255},
                    "Start a fresh homunculus and march into the first zone.");

    DrawButtonChrome(renderer_, startButton, SDL_Color{108, 136, 104, 255},
                     SDL_Color{236, 232, 224, 255}, SDL_Color{164, 204, 152, 255}, "Start Demo",
                     true);

    DrawTextFit(renderer_, footerPanel.x + 16.0F, footerPanel.y + 132.0F, footerPanel.w - 32.0F,
                SDL_Color{174, 168, 160, 255},
...
void FleshgolemGame::RenderResourcePanel(const SDL_FRect& panel) const {
    DrawPanelChrome(renderer_, panel, SDL_Color{32, 24, 34, 240}, SDL_Color{118, 96, 118, 255},
                    SDL_Color{144, 92, 118, 255});
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 12.0F, SDL_Color{234, 214, 214, 255},
             "FLESHGOLEM DEMO");
    DrawTextFit(renderer_, panel.x + 180.0F, panel.y + 12.0F, 420.0F, SDL_Color{194, 188, 182, 255},
                "Space pause  K tree  R restart  E dissolve  N scout");
    DrawTag(renderer_, SDL_FRect{panel.x + 940.0F, panel.y + 10.0F, 146.0F, 24.0F}, zone.accent,
            SDL_Color{236, 232, 224, 255}, zone.name);
    DrawTag(renderer_, SDL_FRect{panel.x + 1096.0F, panel.y + 10.0F, 120.0F, 24.0F},
    ...
    resourceLine << "Wave " << (runState.zoneEncounter + 1) << "/" << runState.encountersPerZone
                 << "   Lv " << runState.level << "   XP " << runState.experience << "/"
                 << runState.experienceToNextLevel << "   SP " << runState.skillPoints << "   K "
                 << resources.kills << "   Bio " << resources.biomass << "   Bone "
    ...
    statusLine << "Grafts " << resources.grafts << "   Salvaged " << resources.salvaged
               << "   Spent " << runState.spentSkillPoints << "   Bosses "
               << runState.bossesDefeated << "   RunEss " << resources.essence << "   Bank "
               << bankedEssence_ << "   Stab " << FormatNumber(stability.demand) << "/"
               << FormatNumber(stability.capacity);
    DrawTextFit(
        renderer_, panel.x + 16.0F, panel.y + 54.0F, panel.w - 32.0F,
...
void FleshgolemGame::RenderPlayerPanel(const SDL_FRect& panel) const {
    DrawPanelChrome(renderer_, panel, SDL_Color{28, 24, 30, 240}, SDL_Color{92, 82, 98, 255},
                    SDL_Color{138, 104, 148, 255});
    ...
    DrawText(renderer_, panel.x + 16.0F, panel.y + 12.0F, SDL_Color{232, 216, 216, 255},
             name.value);
    DrawTag(renderer_, SDL_FRect{panel.x + 330.0F, panel.y + 10.0F, 118.0F, 24.0F},
            SDL_Color{92, 104, 126, 255}, SDL_Color{232, 228, 220, 255}, "Body status",
            SDL_Color{232, 232, 228, 255});
    ...
    DrawTextFit(renderer_, panel.x + 246.0F, panel.y + 36.0F, 180.0F, SDL_Color{214, 196, 192, 255},
                hpLine.str());
    ...
    DrawTextFit(renderer_, panel.x + 16.0F, panel.y + 60.0F, 430.0F, SDL_Color{186, 194, 182, 255},
                statLineOne.str());
    ...
    DrawTextFit(renderer_, panel.x + 16.0F, panel.y + 78.0F, 430.0F, SDL_Color{186, 194, 182, 255},
                statLineTwo.str());
    ...
    DrawTextFit(renderer_, panel.x + 16.0F, panel.y + 96.0F, 430.0F, SDL_Color{172, 186, 202, 255},
                combatLine.str());
    ...
    DrawTextFit(
        renderer_, panel.x + 16.0F, panel.y + 114.0F, 430.0F,
        stability.overload > 0.0F ? SDL_Color{214, 108, 98, 255} : SDL_Color{144, 182, 144, 255},
        stabilityLine.str());

    FillRect(renderer_, SDL_FRect{panel.x + 14.0F, panel.y + 146.0F, 442.0F, 1.0F},
    ...
    for (const SlotType slot : kAllSlots) {
        ...
        DrawTextFit(renderer_, panel.x + 278.0F, textY, 176.0F, textColor, headerLine.str());
        DrawWrappedText(renderer_, panel.x + 278.0F, textY + 14.0F, 176.0F, 2, 14.0F, textColor,
                        detailLine);
        textY += 30.0F;
    }
}
...
void FleshgolemGame::RenderEncounterPanel(const SDL_FRect& panel) const {
    const RunState& runState = registry_.get<RunState>(player_);
    const ZoneDefinition& zone = ZoneDefinitionFor(runState.zoneIndex);
    DrawPanelChrome(renderer_, panel, SDL_Color{30, 22, 26, 240}, SDL_Color{102, 80, 90, 255},
                    zone.accent);

    DrawText(renderer_, panel.x + 16.0F, panel.y + 12.0F, SDL_Color{232, 216, 216, 255}, zone.name);
    DrawTag(renderer_, SDL_FRect{panel.x + 134.0F, panel.y + 10.0F, 104.0F, 24.0F}, zone.accent,
            SDL_Color{232, 228, 220, 255}, "Encounter");
    DrawWrappedText(renderer_, panel.x + 250.0F, panel.y + 12.0F, panel.w - 266.0F, 2, 16.0F,
                    SDL_Color{190, 184, 178, 255}, zone.subtitle);

    const SDL_FRect laneRect{panel.x + 16.0F, panel.y + 54.0F, panel.w - 32.0F, 194.0F};
    RenderLane(laneRect);
    ...
    DrawText(renderer_, panel.x + 366.0F, panel.y + 258.0F, SDL_Color{212, 202, 194, 255},
             "Zone progress");
    ...
    if (currentEnemy_ != entt::null && registry_.valid(currentEnemy_)) {
        ...
        std::ostringstream line;
        line << "T" << enemy.tier << "  " << (combatJoined_ ? "CONTACT" : "APPROACHING");
        if (enemy.elite) {
            line << "  ELITE";
        }
        if (enemy.boss) {
        ...
        DrawTextFit(renderer_, panel.x + 18.0F, panel.y + 314.0F, panel.w - 36.0F,
                    SDL_Color{196, 188, 182, 255}, line.str());
    } else if (currentCorpse_ != entt::null && registry_.valid(currentCorpse_)) {
        const Corpse& corpse = registry_.get<Corpse>(currentCorpse_);
        DrawTextFit(renderer_, panel.x + 18.0F, panel.y + 278.0F, 390.0F,
                    SDL_Color{208, 204, 194, 255}, registry_.get<Name>(currentCorpse_).value);
        DrawBar(renderer_, SDL_FRect{panel.x + 18.0F, panel.y + 296.0F, 260.0F, 12.0F},
                1.0F - corpse.harvestTimer / std::max(0.01F, corpse.maxHarvestTimer),
                SDL_Color{112, 172, 96, 255}, SDL_Color{50, 74, 46, 255});
        DrawWrappedText(renderer_, panel.x + 18.0F, panel.y + 314.0F, panel.w - 36.0F, 2, 16.0F,
                        SDL_Color{180, 196, 162, 255},
                        corpse.boss
                            ? "Boss corpse: harvesting pushes the run into the next phase."
                            : "Auto-harvesting the corpse for resources, salvage, and momentum.");
    } else {
        DrawWrappedText(renderer_, panel.x + 18.0F, panel.y + 278.0F, panel.w - 36.0F, 2, 16.0F,
                        SDL_Color{208, 198, 188, 255},
                        "The homunculus keeps walking while the next enemy approaches.");
        DrawWrappedText(renderer_, panel.x + 18.0F, panel.y + 310.0F, panel.w - 36.0F, 2, 16.0F,
...
void FleshgolemGame::RenderLane(const SDL_FRect& laneRect) const {
    ...
    DrawWrappedText(
        renderer_, laneRect.x + 24.0F, laneRect.y + 10.0F, laneRect.w - 48.0F, 2, 16.0F,
        SDL_Color{210, 204, 198, 255},
        "Walk in place, gain levels, and open the skill tree whenever points are ready.");
...
void FleshgolemGame::RenderEventLog(const SDL_FRect& panel) const {
    DrawPanelChrome(renderer_, panel, SDL_Color{24, 22, 24, 240}, SDL_Color{90, 82, 88, 255},
                    SDL_Color{108, 90, 116, 255});
    ...
    for (const std::string& line : eventLog_) {
        FillRect(renderer_, SDL_FRect{panel.x + 12.0F, y - 4.0F, panel.w - 24.0F, 16.0F},
                 lineIndex == 0 ? SDL_Color{72, 56, 66, 120} : SDL_Color{42, 36, 44, 92});
        const SDL_Color color =
            (lineIndex == 0) ? SDL_Color{220, 210, 198, 255} : SDL_Color{166, 166, 166, 255};
        DrawTextFit(renderer_, panel.x + 16.0F, y, panel.w - 32.0F, color, line);
        y += 18.0F;
        ++lineIndex;
    }
}
...
void FleshgolemGame::RenderPauseOverlay() const {
    ...
    FillRect(renderer_, SDL_FRect{0.0F, 0.0F, kWindowWidth, kWindowHeight},
             SDL_Color{0, 0, 0, 150});
    DrawPanelChrome(renderer_, modal, SDL_Color{40, 28, 36, 245}, SDL_Color{188, 172, 168, 255},
    ...
    DrawTextFit(renderer_, modal.x + 74.0F, modal.y + 94.0F, modal.w - 148.0F,
                SDL_Color{176, 168, 162, 255}, "Press Space or click a button to continue.");

    DrawButtonChrome(renderer_, resumeButton, SDL_Color{92, 132, 108, 255},
                     SDL_Color{228, 228, 220, 255}, SDL_Color{166, 202, 164, 255}, "Resume", true);

    DrawButtonChrome(renderer_, restartButton, SDL_Color{142, 84, 90, 255},
                     SDL_Color{228, 228, 220, 255}, SDL_Color{206, 138, 144, 255}, "Restart");
}
...
void FleshgolemGame::RenderSkillTreeOverlay() const {
    ...
    const SkillNodeDefinition& selectedDefinition = SkillDefinition(selectedSkillIndex_);
    const int selectedRank = runState.skillRanks[static_cast<std::size_t>(selectedSkillIndex_)];
    const bool selectedUnlocked = SkillPrerequisitesMet(selectedDefinition, runState.skillRanks);
    const bool selectedCanSpend =
        selectedUnlocked && selectedRank < selectedDefinition.maxRank && runState.skillPoints > 0;

    DrawPanelChrome(renderer_, modal, SDL_Color{26, 20, 28, 245}, SDL_Color{196, 188, 180, 255},
                    SDL_Color{146, 104, 162, 255});
    DrawPanelChrome(renderer_, graph, SDL_Color{24, 22, 28, 238}, SDL_Color{98, 90, 112, 255},
                    SDL_Color{104, 126, 174, 255});
    DrawPanelChrome(renderer_, details, SDL_Color{28, 24, 30, 238}, SDL_Color{98, 90, 112, 255},
                    SDL_Color{180, 126, 92, 255});

    DrawText(renderer_, modal.x + 26.0F, modal.y + 18.0F, SDL_Color{242, 232, 224, 255},
             "SKILL TREE");

    std::ostringstream levelLine;
    levelLine << "Level " << runState.level << "   XP " << runState.experience << "/"
              << runState.experienceToNextLevel << "   Unspent points " << runState.skillPoints;
    DrawTextFit(renderer_, modal.x + 170.0F, modal.y + 20.0F, 560.0F, SDL_Color{212, 204, 196, 255},
                levelLine.str());
    DrawTextFit(renderer_, modal.x + 26.0F, modal.y + 44.0F, modal.w - 52.0F,
                SDL_Color{180, 184, 192, 255},
                "Arrow keys move, Enter spends, K or Esc closes. Mouse can select nodes and use "
                "the buttons.");

    const float xpRatio = static_cast<float>(runState.experience) /
                          static_cast<float>(std::max(1, runState.experienceToNextLevel));
    DrawBar(renderer_, SDL_FRect{modal.x + 738.0F, modal.y + 22.0F, 294.0F, 10.0F}, xpRatio,
            SDL_Color{110, 174, 126, 255}, SDL_Color{52, 56, 62, 255});
    ...
    for (int skillIndex = 0; skillIndex < static_cast<int>(kSkillNodeCount); ++skillIndex) {
        ...
        for (int prerequisiteIndex = 0; prerequisiteIndex < definition.prerequisiteCount;
             ++prerequisiteIndex) {
            const int prerequisite =
                definition.prerequisites[static_cast<std::size_t>(prerequisiteIndex)];
            if (prerequisite < 0) {
                continue;
            }
            const SDL_FPoint end = SkillNodeCenter(prerequisite);
            const bool active = runState.skillRanks[static_cast<std::size_t>(prerequisite)] > 0;
            const SDL_Color lineColor =
                active ? SDL_Color{188, 174, 128, 255} : SDL_Color{76, 72, 82, 255};
            const SDL_FRect horizontal{std::min(start.x, end.x), start.y - 1.0F,
                                       std::fabs(start.x - end.x), 3.0F};
            const SDL_FRect vertical{end.x - 1.0F, std::min(start.y, end.y), 3.0F,
                                     std::fabs(start.y - end.y)};
    ...
    for (int skillIndex = 0; skillIndex < static_cast<int>(kSkillNodeCount); ++skillIndex) {
        ...
        const bool unlocked = SkillPrerequisitesMet(definition, runState.skillRanks);
        const bool maxed = rank >= definition.maxRank;
        const bool selected = skillIndex == selectedSkillIndex_;

        SDL_Color fill = unlocked ? SDL_Color{32, 28, 34, 236} : SDL_Color{18, 18, 22, 236};
        SDL_Color border = unlocked ? definition.color : SDL_Color{88, 84, 96, 255};
        if (selected) {
        ...
        DrawPanelChrome(renderer_, nodeRect, fill, border,
                        unlocked ? definition.color : SDL_Color{70, 68, 78, 255});
        DrawTag(renderer_, SDL_FRect{nodeRect.x + 10.0F, nodeRect.y + 10.0F, 42.0F, 18.0F},
                unlocked ? definition.color : SDL_Color{84, 84, 90, 255},
                unlocked ? definition.color : SDL_Color{84, 84, 90, 255}, definition.shortName);
        DrawTextFit(renderer_, nodeRect.x + 60.0F, nodeRect.y + 12.0F, nodeRect.w - 70.0F,
                    unlocked ? SDL_Color{236, 228, 220, 255} : SDL_Color{152, 148, 156, 255},
                    definition.name);

        std::ostringstream rankLine;
        rankLine << "Rank " << rank << "/" << definition.maxRank;
        DrawTextFit(renderer_, nodeRect.x + 10.0F, nodeRect.y + 36.0F, nodeRect.w - 20.0F,
                    maxed      ? SDL_Color{220, 202, 156, 255}
                    : unlocked ? SDL_Color{188, 194, 202, 255}
                               : SDL_Color{132, 130, 138, 255},
                    rankLine.str());

        DrawWrappedText(renderer_, nodeRect.x + 10.0F, nodeRect.y + 56.0F, nodeRect.w - 20.0F, 2,
                        14.0F,
                        unlocked ? SDL_Color{174, 170, 164, 255} : SDL_Color{114, 112, 120, 255},
                        BuildSkillNodeSummary(definition));
    }

    DrawText(renderer_, details.x + 18.0F, details.y + 16.0F, SDL_Color{238, 230, 222, 255},
             selectedDefinition.name);

    std::ostringstream detailRankLine;
    detailRankLine << "Rank " << selectedRank << "/" << selectedDefinition.maxRank;
    DrawTextFit(renderer_, details.x + 18.0F, details.y + 40.0F, details.w - 36.0F,
                SDL_Color{192, 198, 208, 255}, detailRankLine.str());

    DrawWrappedText(renderer_, details.x + 18.0F, details.y + 68.0F, details.w - 36.0F, 4, 16.0F,
                    SDL_Color{210, 202, 194, 255}, selectedDefinition.description);
    DrawWrappedText(renderer_, details.x + 18.0F, details.y + 144.0F, details.w - 36.0F, 4, 16.0F,
                    SDL_Color{178, 196, 172, 255}, BuildSkillNodeSummary(selectedDefinition));
    ...
    if (selectedDefinition.prerequisiteCount == 0) {
        ...
        prereqLine << "Prereqs:";
        for (int index = 0; index < selectedDefinition.prerequisiteCount; ++index) {
            const int prerequisite =
                selectedDefinition.prerequisites[static_cast<std::size_t>(index)];
            if (prerequisite < 0) {
                continue;
            }
            prereqLine << (index == 0 ? " " : ", ") << SkillDefinition(prerequisite).name;
        }
    }
    DrawWrappedText(
        renderer_, details.x + 18.0F, details.y + 220.0F, details.w - 36.0F, 4, 16.0F,
        selectedUnlocked ? SDL_Color{188, 188, 182, 255} : SDL_Color{214, 142, 132, 255},
        prereqLine.str());

    DrawWrappedText(renderer_, details.x + 18.0F, details.y + 286.0F, details.w - 36.0F, 5, 16.0F,
                    SDL_Color{172, 180, 192, 255},
                    selectedCanSpend ? "This node can take a point right now."
                    : selectedRank >= selectedDefinition.maxRank ? "This node is already maxed."
                    : runState.skillPoints <= 0 ? "Gain another level to earn a point."
                                                : "Unlock the prerequisite branches first.");

    DrawButtonChrome(renderer_, spendButton,
                     selectedCanSpend ? SDL_Color{98, 140, 106, 255} : SDL_Color{68, 72, 78, 255},
                     SDL_Color{228, 228, 220, 255},
                     selectedCanSpend ? SDL_Color{166, 202, 164, 255} : SDL_Color{96, 96, 100, 255},
                     selectedCanSpend ? "Spend Point" : "Cannot Spend", selectedCanSpend);
    DrawButtonChrome(renderer_, closeButton, SDL_Color{86, 92, 122, 255},
                     SDL_Color{236, 232, 224, 255}, SDL_Color{142, 150, 188, 255}, "Return to Run");
}
...
void FleshgolemGame::RenderRunEndOverlay() const {
    ...
    FillRect(renderer_, SDL_FRect{0.0F, 0.0F, kWindowWidth, kWindowHeight},
             SDL_Color{0, 0, 0, 176});
    DrawPanelChrome(renderer_, modal, SDL_Color{34, 26, 34, 245}, SDL_Color{206, 192, 180, 255},
                    sceneState_ == SceneState::Victory ? SDL_Color{142, 198, 152, 255}
    ...
    DrawWrappedText(renderer_, modal.x + 48.0F, modal.y + 70.0F, modal.w - 96.0F, 2, 18.0F,
                    SDL_Color{212, 204, 196, 255}, endSubtitle_);
    ...
    DrawWrappedText(renderer_, modal.x + 54.0F, modal.y + 206.0F, modal.w - 108.0F, 3, 18.0F,
                    SDL_Color{186, 182, 176, 255},
                    sceneState_ == SceneState::Victory
                        ? "The demo run is complete. Start another run or return to the title "
                          "screen."
                        : "Retry immediately, or return to the title screen to review your banked "
                          "Essence.");
    if (sceneState_ == SceneState::Defeat) {
        DrawWrappedText(renderer_, modal.x + 54.0F, modal.y + 260.0F, modal.w - 108.0F, 2, 18.0F,
    ...
    DrawButtonChrome(renderer_, primaryButton, SDL_Color{108, 136, 104, 255},
                     SDL_Color{236, 232, 224, 255}, SDL_Color{166, 202, 164, 255}, "Start New Run",
                     true);

    DrawButtonChrome(renderer_, secondaryButton, SDL_Color{86, 92, 122, 255},
                     SDL_Color{236, 232, 224, 255}, SDL_Color{142, 150, 188, 255},
                     "Return to Title");
}
